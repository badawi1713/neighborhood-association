...
function ListFilterDialog({ open, closeDialogHandler, module }) {
  const dispatch = useDispatch();
  const { arisanMeetingsFilterFormData, loadingDialog } = useSelector(
  ...
  const formMethods = useForm({
    ...
    defaultValues: useMemo(() => {
      return arisanMeetingsFilterFormData;
  ...
  async function handleFilter() {
    ...
    const response = await dispatch(getArisanMeetings());
    if (response) {
      closeDialogHandler();
    }
    await dispatch(
...
ListFilterDialog.propTypes = {
  closeDialogHandler: PropTypes.func.isRequired,
  open: PropTypes.bool,
};

ListFilterDialog.defaultProps = {
  closeDialogHandler: () => {
