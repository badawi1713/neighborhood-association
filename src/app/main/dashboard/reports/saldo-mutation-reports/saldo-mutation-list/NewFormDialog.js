...
function NewFormDialog({ open, closeDialogHandler }) {
  ...
  const { loadingPost } = useSelector((state) => state.saldoMutationReportsReducer);
  ...
  const handleSave = handleSubmit(async (data) => {
    ...
    const response = await dispatch(postSaldoMutationReports(payload));
    if (response) {
      closeDialogHandler();
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, ...field } }) => (
                      <TextField
                        ...
                        label="Nama SaldoMutationReports"
                        id="name"
                        variant="outlined"
                        fullWidth
                        helperText={errors?.users?.[userIndex]?.name?.message}
                        placeholder="Nama SaldoMutationReports"
                      />
                    )}
                  />
                  <Controller
