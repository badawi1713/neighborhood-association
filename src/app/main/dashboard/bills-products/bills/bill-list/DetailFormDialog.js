...
function DetailFormDialog({ open, closeDialogHandler }) {
  const { billDetailData } = useSelector((state) => state.billReducer);

  const rows = useMemo(() => {
    return billDetailData?.tagihan?.list_data?.length > 0
      ? billDetailData?.tagihan?.list_data?.map((bill) => {
          return createData(bill?.nama, bill?.nominal, bill?.detail_id);
        })
      : [];
  }, [billDetailData]);
                      ...
                      <TableBody>
                        ...
                        <StyledTableRow>
                          ...
                          <TableCell align="left" colSpan={1}>
                            Rp{currencyFormat(billDetailData?.tagihan?.total_nominal || 0)}
                          </TableCell>
                        </StyledTableRow>
                      </TableBody>
                    </Table>
