...
import UserListFilterDialog from './UserListFilterDialog';
import NewFormDialog from './NewFormDialog';

function UserListHeader() {
  const { userManagementUserNameSearch, userManagementUserFilterFormData } = useSelector(
    (state) => state.userManagementUserReducer
  );
  const { filterDate, paymentType } = userManagementUserFilterFormData;
          ...
          >
            ...
            <form onSubmit={handleSearchData} className="w-full">
              <Input
                ...
                disableUnderline
                fullWidth
                value={userManagementUserNameSearch}
                inputProps={{
                  'aria-label': 'Search',
                }}
                onChange={(ev) =>
                  dispatch(
                    changeUserManagementUserReducer({
                      userManagementUserNameSearch: ev.target.value,
                    })
                  )
                }
                endAdornment={
