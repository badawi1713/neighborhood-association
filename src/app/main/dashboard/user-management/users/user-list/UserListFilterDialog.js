...
function UserListFilterDialog({ open, closeDialogHandler, module }) {
  const dispatch = useDispatch();
  const { userManagementUserFilterFormData, loadingDialog } = useSelector(
  ...
  const formMethods = useForm({
    ...
    defaultValues: useMemo(() => {
      return userManagementUserFilterFormData;
  ...
  async function handleFilter() {
    ...
    const response = await dispatch(getUserManagementUser());
    if (response) {
      closeDialogHandler();
    }
    await dispatch(
...
UserListFilterDialog.propTypes = {
  closeDialogHandler: PropTypes.func.isRequired,
  open: PropTypes.bool,
};

UserListFilterDialog.defaultProps = {
  closeDialogHandler: () => {
