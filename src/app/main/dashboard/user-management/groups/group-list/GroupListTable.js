...
import {
  ...
  deleteUserManagementGroup,
  getUserManagementGroup,
  getUserManagementGroupById,
...
import GroupListTableHead from './GroupListTableHead';
...
function GroupListTable(props) {
  ...
  const {
    ...
    userManagementGroupTotal,
    loading,
    loadingDelete,
    loadingDialog,
  } = useSelector((state) => state.userManagementGroupReducer);
  ...
  async function handleChangeRowsPerPage(event) {
    await dispatch(
      changeUserManagementGroupReducer({
        ...
        userManagementGroupLimit: event.target.value,
      })
    );
    await dispatch(getUserManagementGroup());
  }
  ...
  if (error) {
    return (
      ...
      >
        ...
        <Button
          className="mt-24"
          onClick={() => dispatch(getUserManagementGroup())}
          variant="outlined"
          color="inherit"
        >
          Coba lagi
  ...
  if (userManagementGroupData?.length === 0) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1, transition: { delay: 0.1 } }}
    ...
    <div className="w-full flex flex-col min-h-full">
      <TableContainer className="grow overflow-x-auto">
        <Table stickyHeader className="min-w-xl whitespace-nowrap" aria-labelledby="tableTitle">
          <GroupListTableHead />
          <TableBody>
            {userManagementGroupData?.map((item, index) => {
              return (
                    ...
                    <div className="flex justify-center gap-3 items-center">
                      <Tooltip arrow title="Edit">
                        <span>
                          <IconButton
                            onClick={async () => {
                              const response = await dispatch(getUserManagementGroupById(item?.id));
                              if (response) {
      ...
      <TablePagination
        ...
        component="div"
        count={userManagementGroupTotal}
        rowsPerPage={userManagementGroupLimit}
        page={userManagementGroupPage}
        backIconButtonProps={{
      ...
      {openDeleteConfirmation && (
        <ConfirmationDialog
          ...
          confirmActionHandler={async () => {
            const response = await dispatch(deleteUserManagementGroup(selectedId));
            if (response) {
...
export default withRouter(GroupListTable);
