...
function GroupListFilterDialog({ open, closeDialogHandler, module }) {
  const dispatch = useDispatch();
  const { userManagementGroupFilterFormData, loadingDialog } = useSelector(
  ...
  const formMethods = useForm({
    ...
    defaultValues: useMemo(() => {
      return userManagementGroupFilterFormData;
  ...
  async function handleFilter() {
    ...
    const response = await dispatch(getUserManagementGroup());
    if (response) {
      closeDialogHandler();
    }
    await dispatch(
...
GroupListFilterDialog.propTypes = {
  closeDialogHandler: PropTypes.func.isRequired,
  open: PropTypes.bool,
};

GroupListFilterDialog.defaultProps = {
  closeDialogHandler: () => {
...
export default GroupListFilterDialog;
