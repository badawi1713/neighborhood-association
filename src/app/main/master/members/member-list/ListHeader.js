...
function ListHeader() {
  const { masterMembersNameSearch, masterMembersFilterFormData } = useSelector(
    (state) => state.masterMembersReducer
  );
  const { filterDate, paymentType } = masterMembersFilterFormData;
            ...
            <form onSubmit={handleSearchData} className="w-full">
              <Input
                ...
                onChange={(ev) =>
                  dispatch(
                    changeMasterMembersReducer({
                      masterMembersNameSearch: ev.target.value,
                    })
                  )
                }
                endAdornment={
